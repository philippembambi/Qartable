...
if (isset($_FILES['uploadFile2']) && $_FILES['uploadFile2']['error'] === 0) 
{
    $extension_upload = strtolower(substr(strrchr($_FILES['uploadFile2']['name'], '.') ,1));
...
else
{
    ...
    if (@move_uploaded_file($_FILES['uploadFile2']['tmp_name'], $targetpath)) 
    { 
        // Si ça fonctionne
    $error = 'OK';
    } 
...
?>
<script>
window.top.window.uploadEnd2("<?php echo $error; ?>", "<?php echo
$filename; ?>");
</script>